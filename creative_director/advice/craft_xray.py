...
_LEVER_VLM_SYSTEM = (
    ...
    "- The SAME SIN IN DISGUISE — do NOT, under the banner of 'pacing' or 'tightening', tell the creator "
    "to CUT, DELETE, SKIP, or 'start past' the tease/suspense/'wait for it' device itself, or to 'open "
    "on the reveal' / 'get to the reveal faster' by removing the build-up. Deleting the deliberate delay "
    "IS front-loading the reveal by another name. Tightening pacing means cutting genuine DEAD AIR or "
    "redundant shots that are NOT part of the setup — never the withhold beat that earns the payoff. If "
    "the only way to 'speed it up' is to remove the suspense, the reel does not have a pacing problem.\n"
